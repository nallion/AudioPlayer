...
namespace AudioVisualizerPlayer.Services
{
    ...
    public class VisualizerService : IDisposable
    {
        ...
        /// <summary>
        /// true — визуализатор пропускает всю работу в OnQuantumStarted
        /// (экономия CPU при блокировке экрана/сворачивании), но соединение
        /// Submix -> FrameOutput остаётся ЖИВЫМ всё это время — топология
        /// графа не меняется. Раньше при блокировке мы делали Dispose()
        /// визуализатора целиком и заново AttachToAsync при разблокировке —
        /// то есть удаляли и добавляли узел прямо в живом, играющем графе.
        /// Именно это, судя по всему, и давало гарантированные щелчки на
        /// lock/unlock — соединение Submix->FrameOutput и так не самое
        /// стабильное место на этой платформе (см. историю с XAUDIO2_E_INVALID_CALL
        /// и гонками при подключении наушников), трогать его на живом графе
        /// лишний раз не стоит. Теперь топология не меняется вообще —
        /// меняется только то, считаем ли мы FFT.
        /// </summary>
        ...
        private void OnQuantumStarted(AudioGraph sender, object args)
        {
            try
            {
                if (IsPaused) return; // экономим CPU при блокировке экрана, не трогая топологию графа
